...
//
// VENDORED (MIT) from wireguard-apple `Sources/Shared/Model/`. The
// `TunnelConfiguration(fromWgQuickConfig:)` wg-quick parser ships in the WireGuard
// *app* sources, not in the WireGuardKit SPM library target, so PacketTunnelProvider
// must carry it to turn core-ts's rendered `.conf` strings into a TunnelConfiguration.
// Kept byte-for-byte from upstream (plus this note + the WireGuardKit import that
// upstream got for free by living in the same module) for easy re-sync.
