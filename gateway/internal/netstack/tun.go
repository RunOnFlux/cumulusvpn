...
// Stack returns the underlying gVisor *stack.Stack. This is the single
// accessor CumulusVPN adds on top of the upstream wireguard-go netstack tun
// (SPDX-License-Identifier: MIT, Copyright (C) WireGuard LLC): the exit
// forwarder needs the stack to register TCP/UDP forwarders and enable
// promiscuous mode + spoofing, which the upstream Net type does not expose.
func (net *Net) Stack() *stack.Stack {
	return (*netTun)(net).stack
}
